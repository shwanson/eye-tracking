...
def filter_quality(df: pd.DataFrame, confidence_threshold: float = 0.8) -> pd.DataFrame:
    ...
    if 'left_gaze_point_validity' in df.columns and 'right_gaze_point_validity' in df.columns:
        ...
        logging.warning("Validity columns not found. Data not filtered.")
        return df
...
def detect_fixations(df: pd.DataFrame, max_dispersion: float = 0.04, 
    ...
    """
    logging.debug('detect_fixations input shape: %s', df.shape)
    logging.debug('detect_fixations input columns: %s', df.columns.tolist())
    fixations = []
    window = []
    start_t = None
    ...
    if 'is_blink' in df.columns:
        df = df[~df['is_blink']]
    logging.debug('detect_fixations after blink filter shape: %s', df.shape)
    logging.debug('detect_fixations after blink filter columns: %s', df.columns.tolist())
    ...
    logging.debug('detect_fixations output shape: %s', result.shape)
    logging.debug('detect_fixations output columns: %s', result.columns.tolist())
    return result
...
def preprocess_pipeline(df: pd.DataFrame, 
    ...
    """
    logging.debug('preprocess_pipeline input shape: %s', df.shape)
    logging.debug('preprocess_pipeline input columns: %s', df.columns.tolist())
    # Filter by quality
    filtered_df = filter_quality(df, confidence_threshold)
    logging.debug('After filter_quality shape: %s', filtered_df.shape)
    logging.debug('After filter_quality columns: %s', filtered_df.columns.tolist())
    # Detect blinks
    if detect_blinks_flag:
        filtered_df = detect_blinks(filtered_df, max_gap_ms)
        logging.debug('After detect_blinks shape: %s', filtered_df.shape)
        logging.debug('After detect_blinks columns: %s', filtered_df.columns.tolist())
    ...
    if detect_fixations_flag:
        ...
        logging.debug('After detect_fixations shape: %s', fixations_df.shape if fixations_df is not None else None)
        logging.debug('After detect_fixations columns: %s', fixations_df.columns.tolist() if fixations_df is not None else None)
